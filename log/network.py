...
class MultiLayerClassification(object):
...
if __name__ == '__main__':
    ...
    print('column_name.shape:', mc_data.column_name.shape)
    input_feature = ['cap-shape_b', 'cap-surface_f', 'cap-color_b', 'bruises_f', 'odor_a', 'gill-attachment_a']
    input_vecter = generate_input_vecter(input_feature, mc_data.column_name)
    print(input_vecter)
    print(input_vecter.shape)
    """
    ...
    network = MultiLayerClassification(input_size=feature_count, hidden_size_list=[100, 10], output_size=2,
                                   weight_init_std='relu', activation='relu',
                                   weight_decay_lambda=1e-6,
                                   use_dropout=True, dropout_ratio=0.05,
                                   use_batchnorm=True)

    # network = MultiLayerClassification(input_size=feature_count, hidden_size_list=[5], output_size=2,
                                   # weight_init_std='relu', activation='relu',
                                   # weight_decay_lambda=1e-6,
